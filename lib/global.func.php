...
function template($file, $tpldir = '')
{
    ...
    $tplfile = WEB_ROOT . '/view/' . $tpldir . '/' . $file . '.html';
    $objfile = WEB_ROOT . '/data/view/' . $tpldir . '_' . $file . '.tpl.php';

    if ('default' != $tpldir && !is_file($tplfile)) {
        $tplfile = WEB_ROOT . '/view/default/' . $file . '.html';
        $objfile = WEB_ROOT . '/data/view/default_' . $file . '.tpl.php';
    }
    if (!file_exists($objfile) || (@filemtime($tplfile) > @filemtime($objfile))) {
        require_once WEB_ROOT . '/lib/template.func.php';
...
function makecode($code, $width = 80, $height = 28, $quality = 3)
{
    ...
    $fontfile = WEB_ROOT . '/css/default/ninab.ttf';
...
function get_resume_dir($uid)
{
    ...
    $dir2 = substr($uid, 3, 3);
    $dir3 = substr($uid, 6, 2);
    $resume_dir = "data/resume/" . $dir1 . '/' . $dir2 . '/' . $dir3 . "/" . $uid;
...
function has_outer($content)
{
    ...
    if (!function_exists('file_get_html')) {
        require_once(WEB_ROOT . '/lib/simple_html_dom.php');
    }
...
EOT;
    ...
    $headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: Tipask1.0 {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=" . WEB_CHARSET . "{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
