...
class servicecontrol extends base {
    ...
    public function onview() {
        ...
        $_ENV['service']->update_view_num($service_id);
        $service = $_ENV['service']->get_by_id($service_id);
        $user_comment = $_ENV['service_comment']->get_by_uid_sid($this->user['uid'], $service_id);
    ...
    private function deal_user_revenue($service_id, $score) {
        if ($score >= 3) {
    ...
    public function onajax_add_comment() {
        ...
        $service_id = $this->post['service_id'];
        $score = min(10, intval($this->post['score']));
        $content = $this->post['content'];
        if (empty($service_id) || $score <= 0 || trim($content) == '') {
            $res['success'] = false;
            $res['error'] = 102; // 参数无效
            echo json_encode($res);
            return;
        ...
        */
        ...
        if (!empty($user_comment)) {
            $res['success'] = false;
            $res['error'] = 103; // 已评论过此服务
            echo json_encode($res);
            return;
        }
        $comment_id = $_ENV['service_comment']->add($service_id, $content, $score,
                                                    $this->user['uid'], $this->user['username']);
        if ($comment_id > 0) {
            $res['success'] = true;
            $res['comment_id'] = $comment_id;
            // deal_user_revenue($service_id, $score);
        } else {
            $res['success'] = false;
            $res['error'] = 104; // 添加操作失败
        }
        echo json_encode($res);
    }
}
