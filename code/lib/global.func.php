...
// url转换器，1为请求转换，就是把类似p-替换为problem/view
// 2为反向转换，就是把类似/problem/view/替换为p-
...
function template($file, $tpldir = '') {
    global $setting;
    $tpldir = ('' == $tpldir) ? 'default' : $tpldir;
    $tplfile = WEB_ROOT . '/code/view/' . $tpldir . '/' . $file . '.html';
    $objfile = WEB_ROOT . '/private/view/' . $tpldir . '_' . $file . '.tpl.php';

    if ('default' != $tpldir && !is_file($tplfile)) {
        $tplfile = WEB_ROOT . '/code/view/default/' . $file . '.html';
        $objfile = WEB_ROOT . '/private/view/default_' . $file . '.tpl.php';
    }
    if (!file_exists($objfile) || (@filemtime($tplfile) > @filemtime($objfile))) {
        require_once WEB_ROOT . '/code/lib/template.func.php';
...
function tcookie($var, $value = 0, $life = 0) {
    global $setting;
    $cookiepre = $setting['cookie_pre'] ? $setting['cookie_pre'] : 'bm_';
    if (0 === $value) {
...
function makecode($code, $width = 80, $height = 28, $quality = 3) {
    ...
    $fontfile = WEB_ROOT . '/public/css/default/ninab.ttf';
...
// 获得用户简历地址
function get_resume_dir($uid) {
    $uid = sprintf("%010d", $uid);
    $dir1 = substr($uid, 0, 3);
    $dir2 = substr($uid, 3, 3);
    $dir3 = substr($uid, 6, 2);
    $resume_dir = "private/userdata/resume" . $dir1 . '/' . $dir2 . '/' . $dir3 . "/" . $uid;

    if (file_exists($resume_dir. ".pdf"))
        return SITE_URL . $resume_dir. ".pdf";
    if (file_exists($resume_dir. ".doc"))
        return SITE_URL . $resume_dir. ".doc";
    if (file_exists($resume_dir. ".docx"))
        return SITE_URL . $resume_dir. ".docx";
...
function url($var, $url = '') {
    ...
    if ($url)
        return SITE_URL . $location; //程序动态获取的，给problem的model使用
    else
...
function has_outer($content) {
    ...
    if (!function_exists('file_get_html')) {
        require_once(WEB_ROOT . '/code/lib/simple_html_dom.php');
    }
...
EOT;
    ...
    if ($mailsend == 3) {
        ...
        $email_from = $from == '' ? '=?' . WEB_CHARSET . '?B?' . base64_encode($setting['site_name']) . "?= <" . $setting['maildefault'] . ">" : (preg_match('/^(.+?) \<(.+?)\>$/', $from, $mats) ? '=?' . WEB_CHARSET . '?B?' . base64_encode($mats[1]) . "?= <$mats[2]>" : $from);
    }
    $email_to = preg_match('/^(.+?) \<(.+?)\>$/', $toemail, $mats) ? ($mailusername ? '=?' . WEB_CHARSET . '?B?' . base64_encode($mats[1]) . "?= <$mats[2]>" : $mats[2]) : $toemail;
    $email_subject = '=?' . WEB_CHARSET . '?B?' . base64_encode(preg_replace("/[\r|\n]/", '', '[' . $setting['site_name'] . '] ' . $subject)) . '?=';
    $email_message = chunk_split(base64_encode(str_replace("\n", "\r\n", str_replace("\r", "\n", str_replace("\r\n", "\n", str_replace("\n\r", "\r", $message))))));

    $headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: Boostme {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=" . WEB_CHARSET . "{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
    ...
    if ($mailsend == 1) {
        ...
        fputs($fp, ($setting['mailauth'] ? 'EHLO' : 'HELO') . " Boostme\r\n");
        $lastmessage = fgets($fp, 512);
...
function resizeThumbnailImage($thumb_image_name, $image, $width, $height, $start_width, $start_height, $scale) {
    ...
    $thumb_image_name = WEB_ROOT . $thumb_image_name;
