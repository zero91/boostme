...
class problemmodel
{
    ...
    {
        $problemlist = array();
        $query = $this->db->query("SELECT * FROM `problem` AS p, problem_tag AS t WHERE p.pid=t.pid AND t.name='$name' AND p.status IN ($status) ORDER BY p.demands DESC,p.time DESC LIMIT $start,$limit");
        while ($problem = $this->db->fetch_array($query)) {
            $problem['format_time'] = tdate($problem['time']);
    ...
    {
        $query = $this->db->query("SELECT * FROM `problem` AS p, problem_tag AS t WHERE p.pid=t.pid AND t.name='$name' AND p.status IN ($status) ORDER BY p.demands DESC");
        return $this->db->num_rows($query);
    }

    // 需求列表，根据指定的status来查询
    function list_by_status($status=PB_STATUS_UNAUDIT, $start = 0, $limit='')
    {
        $problemlist = array();
        $sql = "SELECT * FROM problem WHERE 1=1 ";
    ...
    function update($pid, $title, $description, $hidanswer, $price, $status, $time=0)
    {
        $overdue_days = intval($this->base->setting['overdue_days']);
    ...
    // 更新需求状态
    function update_status($pid, $status=PB_STATUS_CLOSED)
    {
    ...
    {
        $this->db->query("UPDATE `problem` set status=" . PB_STATUS_UNSOLVED . " WHERE status=0 AND `pid` in ($pids)");
    }
    ...
    //根据标题搜索问题的结果数
    function search_title_num($title, $status="1,2")
    {
        $problemnum = 0;
        if ($this->base->setting['xunsearch_open']) {
    ...
    //根据标题搜索问题
    function search_title($title, $status = '1,2', $start = 0, $limit = 20)
    {
        $problemlist = array();
        if ($this->base->setting['xunsearch_open']) {
