    ...
    for course_dict in course_list_dict:
        ...
        for course in course_dict['item']:
            if course_cnt % NUM_PER_LINE == 0:
                panel += "<tr>"
            panel += '''<td width="''' + str(99.0 / NUM_PER_LINE) + '''%"><a href="{SITE_URL}?material/category/''' + course['id'] + '''">''' + course['name'] + '''</a></td>'''
    ...
    for school_dict in school_list_dict:
        ...
        for school in school_dict['item']:
            if school_cnt % NUM_PER_LINE == 0:
                panel += "<tr>"
            panel += '''<td><a href="{SITE_URL}?material/category/''' + school['id'] + '''">''' + school['name'] + '''</a></td>''' 
            school_cnt += 1
            if school_cnt % NUM_PER_LINE == 0:
                panel += "</tr>"
